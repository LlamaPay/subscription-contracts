...
contract SubsFactory is Owned {
    ...
    function createContract(uint _periodDuration, address _vault, address _feeCollector, uint _currentPeriod,
      address rewardRecipient_, address stakingRewards_, address owner_) onlyOwner() external returns (Subs subsContract) {
        ...
        uint8 decimals = subsContract.asset().decimals();
        ...
        */
        token.safeTransferFrom(msg.sender, address(this), unit);
        token.approve(address(subsContract), unit);
        subsContract.subscribeForNextPeriod(address(this), unit, 0, unit);
