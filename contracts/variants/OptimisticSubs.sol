...
contract OptimisticSubs is BoringBatchable {
    ...
    struct Sub {
        address owner; uint initialPeriod; uint expirationDate; uint amountPerCycle; address receiver; uint256 accumulator; uint256 initialShares;
    }
    ...
    function subscribe(address receiver, uint amountPerCycle, uint256 cycles) external {
        ...
        require(subs[subId].owner == address(0), "duplicated sub");
        subs[subId] = Sub(msg.sender, currentPeriod, expiration, amountPerCycle, receiver, sharesAccumulator, sharesLeft);
        emit NewSubscription(msg.sender, currentPeriod, expiration, amountPerCycle, receiver, sharesAccumulator, sharesLeft, subId);
    }

    function unsubscribe(bytes32 subId) external {
        _updateGlobal();
        Sub storage sub = subs[subId];
        require(sub.owner == msg.sender, "wrong sub");
        if(sub.expirationDate > block.timestamp){
            // Most common case, solved in O(1)
            uint sharesPaid = ((sharesAccumulator - sub.accumulator) * sub.amountPerCycle) / DIVISOR;
            uint sharesLeft = sub.initialShares - sharesPaid;
            vault.redeem(sharesLeft, msg.sender, address(this));
            receiverAmountToExpire[sub.receiver][sub.expirationDate] -= sub.amountPerCycle;
            receiverAmountToExpire[sub.receiver][currentPeriod] += sub.amountPerCycle;
        } else {
            // Uncommon case, its just claiming yield generated after sub expired
            uint subsetAccumulator = 0;
            while(sub.initialPeriod < sub.expirationDate){
                subsetAccumulator += sharesPerPeriod[sub.initialPeriod];
                sub.initialPeriod += periodDuration;
            }
            vault.redeem(sub.initialShares - ((subsetAccumulator * sub.amountPerCycle) / DIVISOR), msg.sender, address(this));
        }
        delete subs[subId];
